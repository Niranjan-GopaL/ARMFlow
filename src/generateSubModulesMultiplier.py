...
def generate():
    ...
    for i in range(1,33):
        for j in range(1,33):
            ...
            fileName='{}_{}_{}_SP_WT_RC_GenMul.v'.format(str(i),str(j),TYPE)
            changeModuleName(fileName, 'Mult_{}_{}'.format(str(i),str(j)), 'NR_{}_{}'.format(str(i),str(j)))


            os.system('mv {}_{}_{}_SP_WT_RC_GenMul.v ../modules/rtl/NR_{}_{}.v'.format(str(i),str(j),str(TYPE),str(i),str(j)))

            '''code=generateMultiplier(i,j)
            file=open('../modules/rtl/NR_{}_{}.v'.format(str(i),str(j)),mode='w',encoding='utf-8')
            file.write(code)
            file.close()'''
