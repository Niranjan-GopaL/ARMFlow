...
def generate_multiplier(a_width, b_width):
    ...
    if a_width_1 == 1 or b_width_1 == 1:
        ...
        sum_width_1 = sum_width_1 - 2
    else:
        sum_width = vast.Width( vast.IntConst(f'{sum_width - 1}'), vast.IntConst('0') )
        sum_width_1 = sum_width_1 - 1

    code = f"""
module nr_{a_width_1}_{b_width_1} (
    input [{a_width_1 - 1}:0] A, 
    input [{b_width_1 - 1}:0] B,
    output [{sum_width_1}:0] P
);
    assign P = A * B;
endmodule
"""
...
def main():
    ...
    # print(generate_multiplier(a_width, b_width))
