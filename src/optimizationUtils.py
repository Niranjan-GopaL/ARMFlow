...
#Determine upper and lower limit for variables
...
def repeatingConfigRecursive(bit):
    ...
    if config_value!= -1:
        ...
        return
def mutate(n_bits, X, unique_configs):
    global Y
    Xp = []
    for item in X:
        if n_bits == 8:
        ...
        while (cv.config_validator(n_bits, np.array(Y)) != True and Y not in unique_configs):
            count_of_bits = {}
...
def generateRandomSolution():
    ...
    for i in range(POPULATION):
        ...
        if(N_BITS==32):
            length=180
        eachSolution = [-1] * length

        X.append(eachSolution)
    nbits = N_BITS
    unique_configs = []
    solution = mutate(n_bits=nbits, X=X, unique_configs=unique_configs)
