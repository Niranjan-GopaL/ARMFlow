...
def generate_config_dict(n_bits, array):
    ...
    for i in range(n_bits, 2, -1):
        ...
        if count_of_bits[i] > 1:
            ...
            if(MUTATION_VERBOSE):
                print("Repeating variable = ", i)
            while count_of_bits[i] != 0:
                count_of_bits[i] -= 1
                if(MUTATION_VERBOSE):
                    print(f"Index={repeating_index}")
                    print(f"Repeating config for bit {i} = {repeating_array[repeating_index]}")
...
def generate_list_config_non_repeating(config_dict, count_of_bits, count_of_bits_return, n_bits, N_BITS, array):
    ...
    if (high_bits == low_bits and high_bits>2):
...
def generate_list_config_repeating(config_dict, count_of_bits_return, n_bits, array):
    ...
    if(MUTATION_VERBOSE):
        print(f"Config of repeating part={config}")
    repeating_index += 1
...
def main():
    ...
    N_BITS = 8
    array = [4, -1, -1, -1, 0, 0, -1, -1, 3, 1, 1, 2, 2, -1, -1, -1]
    # print(len(array))
    temp_array = [-1]*(16 - len(array))
